...
u8   KPD_u8GetKey()
{
	...
	for( u8 col =0 ; col <COLNUM; col++)
	{
		...
		DIO_voidSetPinVal(KPD_PORT,col, DIO_PIN_LOW);
		
		for ( u8 row = ROWSNUM ; row < COLNUM+ROWSNUM; row++ ) // 4 -> 7
		{
			
			if ( DIO_u8GetPinVal(KPD_PORT,row ) == DIO_PIN_LOW)
			{
				key = KPD_GRID [row-ROWSNUM][col];
			}
			
			while(DIO_u8GetPinVal(KPD_PORT,row ) == DIO_PIN_LOW);
			_delay_ms(10);
		}
			
		DIO_voidSetPinVal(KPD_PORT,col, DIO_PIN_HIGH);
	}
	return key;
}
